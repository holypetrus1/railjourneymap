...
const state = {
  ...
  journeys: [],
  selectedIndex: null,
  journeyMeasurements: new WeakMap(),
...
function setDefaultDeparture() {
  ...
  const localIso = new Date(date.getTime() - date.getTimezoneOffset() * 60_000)
    .toISOString()
    .slice(0, 16);
  elements.departureInput.value = localIso;
}
...
function setLoading(loading) {
  ...
  elements.searchButton.querySelector('span').textContent = loading
    ? 'Strecken werden geladen …'
    : 'Verbindungen anzeigen';
}

function debounce(callback, wait = 260) {
  let timeout;
  return (...args) => {
...
function setupStationAutocomplete(key) {
  ...
  const render = (stations) => {
    ...
    if (!stations.length) {
      ...
      stations.forEach((station, index) => {
        ...
        meta.textContent = station.id ? `Bahnhof · ${station.id}` : 'Bahnhof';
  ...
  const load = debounce(async () => {
    ...
    try {
      ...
      suggestions.innerHTML = '<div class="suggestion-empty">Suche derzeit nicht verfügbar</div>';
    }
  });
...
function formatTime(value) {
  ...
  return Number.isNaN(date.getTime()) ? '–' : formatter.time.format(date);
}

function formatDate(value) {
  if (!value) return '';
  const date = new Date(value);
  return Number.isNaN(date.getTime()) ? '' : formatter.date.format(date);
}
...
function journeyMeasurement(journey) {
  if (!state.journeyMeasurements.has(journey)) {
    state.journeyMeasurements.set(journey, measureJourney(journey));
  }
  return state.journeyMeasurements.get(journey);
}
...
function renderResults() {
  ...
  state.journeys.forEach((journey, index) => {
    ...
    const duration = journeyDurationMinutes(journey);
    const transfers = journeyTransfers(journey);
    const labels = journeyTrainLabels(journey);
    ...
    timeStrong.textContent = `${formatTime(departure)} – ${formatTime(arrival)}`;
    const dateSpan = document.createElement('span');
    dateSpan.textContent = formatDate(departure);
    ...
    meta.innerHTML = `<span>${formatDuration(duration)}</span><span>${transferLabel(transfers)}</span>`;
...
function makeMarkerIcon(kind, label = '') {
  return window.L.divIcon({
    ...
    html: label ? `<span>${label}</span>` : '<span></span>',
...
function addStationMarker(layer, place, kind, label) {
  ...
  const marker = window.L.marker(coordinates, { icon: makeMarkerIcon(kind, label), keyboard: false });
...
function selectJourney(index, userInitiated = false) {
  ...
  measurement.segments.forEach((segment) => {
    ...
    routeLine.bindTooltip(`${railLegLabel(segment.leg)} · ${formatDistance(segment.distanceMeters, segment.approximate)}`, {
      sticky: true,
    });
  });
  ...
  state.routeBounds = measurement.allPoints.length >= 2
    ? L.latLngBounds(measurement.allPoints)
    : null;
  fitSelectedRoute();
...
async function runSearch() {
  ...
  try {
    ...
    state.journeyMeasurements = new WeakMap();
...
elements.swapButton.addEventListener('click', () => {
  ...
  if (state.stations.from) elements.fromInput.dataset.stationId = state.stations.from.id;
  else delete elements.fromInput.dataset.stationId;
  if (state.stations.to) elements.toInput.dataset.stationId = state.stations.to.id;
  else delete elements.toInput.dataset.stationId;
});

elements.exampleButton.addEventListener('click', () => {
  stationFields.from.select({
    type: 'stop',
    id: '8011160',
    name: 'Berlin Hbf',
    location: { latitude: 52.525083, longitude: 13.369402 },
  });
  stationFields.to.select({
    type: 'stop',
    id: '8002549',
    name: 'Hamburg Hbf',
    location: { latitude: 53.552733, longitude: 10.006909 },
  });
  runSearch();
});
